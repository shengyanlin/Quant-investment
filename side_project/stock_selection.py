...
for stock in stocklist:
    ...
    # 財務分析資料
    url = "https://mops.twse.com.tw/mops/web/t05st22_q1"

    session = requests.session()

    r = session.post(url,
                     {'encodeURIComponent': 1, 'step': 1, 'firstin': 1, 'off': 1, 'isnew': 'true', 'co_id': str(stock)},
                     headers=he)
    ...
    for i in range(len(re)):
        print("This is ", i, "th table")
        print(re[i])
    """
    fin = re[11]

    fin.to_csv("fin.csv",encoding = "utf_8_sig") 
    """
