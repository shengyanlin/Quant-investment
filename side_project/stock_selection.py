...
for year in range(110, 111):
    for month in range(1, 7):
        ...
        rfinal = pd.DataFrame(columns=["公司代號", "上月比較", "去年比較", "年累計比較"])

        for i in range(2, len(re) - 3, 2):
            ...
            stock = fin[["Unnamed: 0_level_0"]]

            mii = mi.iloc[:, [3, 4]]
            yii = yi.iloc[:, [2]]

            finstock = stock.iloc[:, [0]]

            final = pd.concat([finstock, mii, yii], axis=1)

            final = final.drop(final.index[len(final) - 1])

            final.columns = ["公司代號", "上月比較", "去年比較", "年累計比較"]

            rfinal = rfinal.append(final, ignore_index=True)
        ...
        conn = sqlite3.connect('月營收.db')
        cursor = conn.cursor()
        tbname = "tb_" + str(year) + "_" + str(month)
        cursor.execute('CREATE TABLE ' + tbname + "(公司代號,上月比較,去年比較,年累計比較)")

        rfinal.to_sql(tbname, conn, if_exists='append', index=False)
        conn.commit()

        # 清空dataframe
        rfinal.drop(rfinal.index, inplace=True)

        print("================End of writing table================")
        print("==================End of ", year, "_", month, "==================")
...
ofct = pd.DataFrame(columns=["公司代號", "現金流量"])
...
for stock in stocklist:
    ...
    try:
        ...
        r = session.post(url, {'encodeURIComponent': 1, 'step': 1, 'firstin': 1, 'off': 1, 'isnew': 'true',
                               'co_id': str(stock)}, headers=he)
        ...
        df = ofc.to_frame(name='A')
        df = df.iat[1, 0]

        new = pd.DataFrame({"公司代號": stock, "現金流量": df}, index=[i])

        ofct = ofct.append(new, ignore_index=True)
...
conn = sqlite3.connect('現金流量.db')
cursor = conn.cursor()
tbname = "latest"
cursor.execute('CREATE TABLE ' + tbname + "(公司代號,現金流量)")

ofct.to_sql(tbname, conn, if_exists='append', index=False)
conn.commit()
...
for year in range(102, 110):
    for season in range(1, 5):
        ...
        result = pd.concat([npp, bi], axis=1)

        # ----------------------Sqlite----------------------
        print("================Begin to write table================")
        conn = sqlite3.connect('綜合損益.db')
        cursor = conn.cursor()
        tbname = "tb_" + str(year) + "_" + str(season)
        cursor.execute('CREATE TABLE ' + tbname + "(公司代號, 本期淨利, 營業利益)")

        result.to_sql(tbname, conn, if_exists='append', index=False)
        conn.commit()
        print("================End of writing table================")

        print("==================End of ", year, "_", season, "==================")
...
final = pd.DataFrame(columns=["公司代號", "流動比率", "負債比率"])
...
for stock in stocklist:
    ...
    try:
        ...
        r = session.post(url, {'encodeURIComponent': 1, 'run': 'Y', 'step': 1, 'TYPEK': 'sii', 'year': str(109),
                               'isnew': 'true', 'co_id': str(stock), 'firstin': 1, 'off': 1, 'ifrs': 'Y'}, headers=he)
        ...
        new = pd.DataFrame.from_dict({"公司代號": [stock], "流動比率": [cr], "負債比率": [dr]})
...
conn = sqlite3.connect('財務分析.db')
cursor = conn.cursor()
tbname = "finance"
cursor.execute('CREATE TABLE ' + tbname + "(公司代號,流動比率,負債比率)")

final.to_sql(tbname, conn, if_exists='append', index=False)
conn.commit()
